...
/*   Updated: 2024/08/28 21:39:03 by miyuu            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_strjoin(char const *s1, char const *s2)//文字列 s の start 文字目から len 文字分をmallocで確保したchar*に入れてそれを返す。
{
	int		len_s1;
	int		len_s2;
	int		i;
...
static char	*extract_line(char **p_store)
{
	...
	origin = *p_store;//元の文字列も先頭ポインター
	*p_store = NULL;
	if (p_nl == NULL || p_nl + 1 == p_end)//改行がないか、改行が最後の文字だったら
		line = ft_substr(origin, 0, p_end - origin);
	else
	{
		line = ft_substr(origin, 0, p_nl - origin + 1);//元の文字列の0文字目から、改行まで
		*p_store = ft_substr(origin, p_nl - origin + 1, p_end - p_nl);//改行以降の文字列
